// electron/print/a5-invoice.js - A5 GST Tax Invoice (Indian format matching A5 portrait style)
'use strict';

/**
 * Generate an A5 GST Tax Invoice HTML.
 ...
 */
function generateA5InvoiceHTML(invoice, company) {
  const c = {
  ...
  items.forEach((item, idx) => {
    ...
    itemsHTML += `<tr>
      ...
      <td class="r">${discPct > 0 ? discPct.toFixed(1) + '%' : ''}</td>`;
  ...
  // A5 fits about 5 rows cleanly. Let's pad empty rows.
  const minRows = 5;
  ...
  itemsHTML += `<tr class="total-row" style="font-weight: bold; border-top: 1.5px solid #000; border-bottom: 2px double #000; background: #f9f9f9;">
    <td colspan="7" class="r" style="padding-right: 8px; font-weight: bold;">Total</td>`;
  ...
  let slabRows = '';
  slabs.forEach(s => {
    if (slabData[s].taxable > 0 || slabData[s].tax > 0) {
      slabRows += `<tr>
  ...
  if (!slabRows) {
    slabRows = `<tr><td colspan="4" class="c">No GST Tax Slabs Active</td></tr>`;
  } else {
  ...
  // Buyer details
  const buyerName = invoice.client_name || 'Walk-in Customer';
  const buyerAddr1 = invoice.client_address_line1 || invoice.client_address || '';
  const buyerAddr2 = invoice.client_address_line2 || '';
  const buyerCity = invoice.client_city || '';
  const buyerState = invoice.client_state || '';
  const buyerGstin = invoice.client_gstin || '';
  const buyerPhone = invoice.client_phone || invoice.client_mobile || '';

  const buyerAddrLines = [buyerAddr1, buyerAddr2].filter(Boolean);
  if (buyerCity) buyerAddrLines.push(buyerCity);
  if (buyerState) buyerAddrLines.push(buyerState);
...
<style>
  @page { size: A5 portrait; margin: 4mm 6mm; }
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .bank-details h4 { font-size: 10.5px; font-weight: 800; margin-bottom: 3px; }
  ...
  .signatory .company-name { font-weight: 900; font-size: 10.5px; }
  .signatory .sig-line { margin-top: 30px; border-top: 1.5px solid #000; padding-top: 3px; font-weight: 700; }
  ...
  .terms h4 { font-size: 9px; font-weight: 800; color: #000; margin-bottom: 2px; }
  ...
  <table class="grid-header">
    <tr>
      <td class="metadata-cell">
        <strong>Invoice No. &nbsp;: &nbsp;${esc(invoice.invoice_number || '')}</strong>
      </td>
    ...
    <tr>
      ...
      <td class="buyer-cell">
        ...
        <div class="buyer-address">
          ${buyerAddrLines.length > 0 ? buyerAddrLines.join(', ') : 'WALK-IN CUSTOMER'}
        </div>
  ...
  <table class="items-table">
    <thead>
      <tr>
        ...
        ${isInterState
          ? `<th style="width:35px;">IGST %</th><th style="width:50px;">IGST</th>`
          : `<th style="width:35px;">SGST %</th><th style="width:45px;">SGST</th>
             <th style="width:35px;">CGST %</th><th style="width:45px;">CGST</th>`
        }
...
module.exports = { generateA5InvoiceHTML, numberToWordsINR };
